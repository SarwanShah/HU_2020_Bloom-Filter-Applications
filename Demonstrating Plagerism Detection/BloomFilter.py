...
class BloomFilter(object): 
    ...
    def loadFromFile(self, filename, numItems = -1, errorProb = -1):
        if numItems == -1 and errorProb == -1:
          assert type(filename) == str and filename.split('.')[1] and filename.split('-')[4], '''The filename should be a string with the relevant extension added, as well necessary details \
          ...
          self.errorProb = float(filterinfo[1])/1000
        ...
        print('Bit-Array Size: ' + str(len(self.bitArray)))
        print('HashesPerItem: ' + str(self.hashCount))
        print('------------------------------------------- ')
        print(' ')

    def sendToFile(self, outFile):
      out = open(outFile.split('.')[0] + '-' + str(self.numItems) + '-' + str(int(self.errorProb*1000)) + '-' + str(self.hashCount) + '-' + str(self.bitArraySize) + '.txt' , 'wb')
